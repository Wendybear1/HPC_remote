...
X_matrice=[]
Y_matrice=[]
directory =r'/fred/oz132/EEG_ML/15minpreictal/ES'
...
# print(df)
df.to_csv("/fred/oz132/EEG_ML/15minpreictal/data_ES_preictal_15min_CNN.csv")

import os
X_matrice=[]
Y_matrice=[]
directory =r'/fred/oz132/EEG_ML/15minpreictal/PNES'
dir_list = list(os.scandir(directory))
dir_list.sort(key=lambda d:d.path)
for entry in dir_list:
    if (entry.path.endswith(".csv")) and entry.is_file():
...
# print(df)
df.to_csv("/fred/oz132/EEG_ML/15minpreictal/data_PNES_preictal_15min_CNN.csv")