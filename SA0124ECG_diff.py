...
X_matrice=[]
Y_matrice=[]
directory =r'/fred/oz132/EEG_ML/45minpreictal/ES'
...
# print(df)
df.to_csv("/fred/oz132/EEG_ML/45minpreictal/data_ES_preictal_45min_CNN.csv")

import os
X_matrice=[]
Y_matrice=[]
directory =r'/fred/oz132/EEG_ML/45minpreictal/PNES'
dir_list = list(os.scandir(directory))
dir_list.sort(key=lambda d:d.path)
for entry in dir_list:
    if (entry.path.endswith(".csv")) and entry.is_file():
...
# print(df)
df.to_csv("/fred/oz132/EEG_ML/45minpreictal/data_PNES_preictal_45min_CNN.csv")
...
X_matrice=[]
Y_matrice=[]
directory =r'/fred/oz132/EEG_ML/60minpreictal/ES'
dir_list = list(os.scandir(directory))
dir_list.sort(key=lambda d:d.path)
for entry in dir_list:
    if (entry.path.endswith(".csv")) and entry.is_file():
...
df['label']=np.ones(len(df))
# print(df)
df.to_csv("/fred/oz132/EEG_ML/60minpreictal/data_ES_preictal_60min_CNN.csv")

import os
X_matrice=[]
Y_matrice=[]
directory =r'/fred/oz132/EEG_ML/60minpreictal/PNES'
dir_list = list(os.scandir(directory))
dir_list.sort(key=lambda d:d.path)
for entry in dir_list:
    if (entry.path.endswith(".csv")) and entry.is_file():
...
df['label']=np.zeros(len(df))
# print(df)
df.to_csv("/fred/oz132/EEG_ML/60minpreictal/data_PNES_preictal_60min_CNN.csv")